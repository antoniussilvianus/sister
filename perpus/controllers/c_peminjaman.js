                ...
                // $.Dialog.title('<i class="fg-white icon-'+(id!=''?'pencil':'plus-2')+'"></i> '+titl); 
                    ...
                    if (typx=='pinjam') {
                        ...
                        if (id=='') { //add
                            ...
                            $('#tgl_kembaliTB').val(selisihKembali());
            ...
            select: function( event, ui ) { // event setelah data terpilih 
                ...
                if (subaksi=='pinjam') {
                    ...
                    if (tipe==1) {
                        ...
                        $('#idmemberTB').html(ui.item.nis);
                        $('#tipememberTB').html(ui.item.nama);
                        ...
                        $('#idmemberTB').html(ui.item.nip);
                        $('#tipememberTB').html(ui.item.nama);
                    }
                    else if (tipe==3) {
                        $('#idmemberTB').html(ui.item.nid);
                        $('#tipememberTB').html(ui.item.nama);
    ...
    function cmbmember(typ,mbr){
        ...
        ajax(u,d).done(function (dt) {
            ...
            if(dt.status!='sukses'){
                ...
                $.each(dt.mtipe, function(id,item){
                    out+='<option '+(mbr!='' && mbr==item.replid?'selected':'')+' value="'+item.replid+'">'+item.mtipe+'</option>';
                });
            }
            if(typ=='filter'){
                $('#memberS').html('<option value="">-SEMUA-</option>'+out);
                cmbwarna('filter','');
            }else{
                $('#memberTB').html('<option value="">-Pilih Grup Modul-</option>'+out);
            }
        });
    }
...
// tanggal pertama bulan ini : dd mm yyyy 
    function selisihKembali() {
        return dateFormatx('id',(dd+7),monthFormat(mm),yyyy);
    }
