<?php
	...
	if(!isset($_POST['aksi'])){
		...
		switch ($_POST['aksi']) {
			// -----------------------------------------------------------------
			case 'tampil':
				$namasiswa  = isset($_POST['namasiswaS'])?filter(trim($_POST['namasiswaS'])):'';
				...
				if($jum!=0){	
					...
					while($res = mysql_fetch_assoc($result)){	
						$btn ='<td>
									<button '.(isAksi('ceksiswa','u')?'onclick="viewFR('.$res['replid'].');"':'disabled').' data-hint="ubah"  >
										<i class="icon-pencil on-left"></i>
									</button>
									<button  '.(isAksi('ceksiswa','d')?'onclick="del('.$res['replid'].');"':'disabled').' data-hint="hapus" onclick="del('.$res['replid'].');">
			...
			// add / edit -----------------------------------------------------------------
			case 'simpan':
				$s = $tb.' set 	ceksiswa	= "'.filter($_POST['ceksiswaTB']).'",
